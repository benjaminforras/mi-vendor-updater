        ...
        if (fs.existsSync(v + '_old.json') && fs.existsSync(v + '.json')) {
            ...
            if (!diff) {
                ...
                for (let link of links) {
                    ...
                    try {
                        await octokit.repos.uploadReleaseAsset({ headers: { "content-length": fs.statSync(file).size, "content-type": "application/zip" }, url: result.data.upload_url, name: file, label: file, file: file });
